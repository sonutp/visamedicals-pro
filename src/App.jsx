...
import { 
  ...
  Shield, CheckCircle2, Sun, Moon
} from 'lucide-react';

export default function App() {
  // Theme Toggle State: 'dark' or 'light'
  const [theme, setTheme] = useState(() => {
    return localStorage.getItem('visamedicals_theme') || 'dark';
  });
  ...
  useEffect(() => {
    ...
    localStorage.setItem('visamedicals_theme', theme);
  }, [theme]);
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${
        ...
        <div className="flex items-center justify-between gap-4 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-xs">
              ...
              <span className="font-medium opacity-75">User Role:</span>
              <span className="text-cyan-400 font-bold font-mono uppercase">{currentRole}</span>
            </div>
          </div>

          {/* Right Header Actions: Dark / Light Mode Toggle + Notifications */}
          <div className="flex items-center gap-3">
            {/* Dark/Light Mode Switcher */}
            <button
              onClick={toggleTheme}
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-bold border border-slate-700 transition-all shadow-sm"
              title="Toggle Dark / Light Mode"
            >
              {theme === 'dark' ? (
                <>
                  <Sun className="w-4 h-4 text-amber-400 animate-spin-slow" />
                  <span className="text-amber-300 hidden sm:inline">Light Mode</span>
                </>
              ) : (
                <>
                  <Moon className="w-4 h-4 text-indigo-500" />
                  <span className="text-indigo-600 hidden sm:inline">Dark Mode</span>
                </>
              )}
            </button>
            ...
            <button
              ...
              className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition-all relative"
      ...
      {/* Main Body with Legacy Treeview Sidebar + Workspace Content */}
      <div className="flex-1 flex max-w-7xl w-full mx-auto px-4 lg:px-8 py-6 gap-6">
        ...
        <aside className="no-print hidden lg:block w-64 flex-shrink-0">
          <div className="sticky top-20 glass-panel p-3 rounded-2xl border border-slate-800/80 space-y-2">
            <div className="px-3 py-1 text-[11px] font-bold uppercase tracking-wider border-b border-slate-800/80 pb-2 opacity-75">
                ...
                return (
                  <div key={catGroup.category} className="space-y-1">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold hover:bg-slate-800/60 transition-all"
                    >
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                                isActive
                                  ? 'bg-cyan-500/20 text-cyan-400 font-bold border border-cyan-500/30 glow-cyan'
                                  : 'opacity-70 hover:opacity-100 hover:bg-slate-800/40'
                              }`}
                            >
                              <SubIcon className={`w-3.5 h-3.5 ${isActive ? 'text-cyan-400' : 'opacity-60'}`} />
          ...
          {legacyMenuStructure.flatMap(g => g.items).map(item => (
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-bold ${
                activeSubMenu === item.id ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-300'
              }`}
            >
              {item.label}
            </button>
